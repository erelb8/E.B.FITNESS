...
(function () {
  ...
  (window.EB_MOD = window.EB_MOD || {})['intake'] = 'v184';
  ...
  function clean(v) {
    var s = String(v == null ? '' : v)
      ...
      // "(ירידה במשקל / מסת שריר / ...)" — האפשרויות, לא התשובה. סוגריים בלי / נשארים: "פציעה בברך (ישנה)"
      .replace(/\([^)]*\/[^)]*\)/g, ' ');
    /* סוגר בלי פותח: שארית של רשימת אפשרויות שנשברה בין שורות, למשל
       "/ כושר כללי / שיקום פציעה)- מסת שריר". התשובה היא מה שאחרי
       הסוגר. כך נכנסו שש מטרות שבורות ב-9.2026, ואחת מהן הייתה ")". */
    var close = s.lastIndexOf(')');
    if (close > -1 && s.indexOf('(') < 0 && s.slice(0, close).indexOf('/') > -1) s = s.slice(close + 1);
    if (!/[0-9A-Za-zא-ת]/.test(s)) return '';   // רק סימנים, כמו ")" — אין כאן תשובה
    return s
      .replace(/^[\s:*•\-–—\/.,]+/, '')
      .replace(/[\s\/.,]+$/, '')
      .replace(/\s+/g, ' ')
