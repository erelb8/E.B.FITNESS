...
const fs = require('fs');
global.window = {};
/* הספרייה נטענת כי הבוט קורא ממנה את סוג התרגיל והציוד — בלעדיה
   הוא היה מחזיר את ברירת המחדל לכל דבר, והבדיקות לא היו בודקות כלום */
...
/* ---------- בכמה להעלות: לפי התרגיל והציוד ---------- */
console.log('=== גודל הקפיצה ===');
t('סקוואט 60 — 2.5 (5 אחוז)',        B.stepFor('סקוואט', 60), 2.5);
t('סקוואט 120 — 5',                  B.stepFor('סקוואט', 120), 5);
t('דדליפט 140 — 5 ולא יותר',         B.stepFor('דדליפט קלאסי', 140), 5);
t('לחיצת חזה 100 — 2.5',             B.stepFor('לחיצת חזה במוט', 100), 2.5);
t('הרחקות צד 8 — קילו',              B.stepFor('הרחקות צד במשקולות', 8), 1);
t('הרחקות צד 14 — שניים',            B.stepFor('הרחקות צד במשקולות', 14), 2);
t('סווינג קטלבל — קופץ ב-4',         B.stepFor('סווינג רוסי', 16), 4);
t('מתח — אין מה להוסיף למוט',        B.stepFor('מתח אחיזה רחבה', 0), 0);
t('חתירה ברצועות — בלי משקל',        B.stepFor('חתירה ברצועות', 0), 0);
t('תרגיל שלא בספרייה — ברירת מחדל',  B.stepFor('תרגיל שהמציא המאמן', 50), 2.5);

// ההמלצה מגיעה עם גודל הקפיצה
B.load({ logs: [5, 2].map(d => log(d, 'יום A', [['סקוואט', 120, 10]])), program: RANGE });
t('סקוואט כבד — ההמלצה 125', /125 ק״ג/.test(B.messages({ skipWeight: true }).map(m => m.text).join(' ')), true);
t('כתוב בכמה עולים', /עלייה של 5/.test(B.messages({ skipWeight: true }).map(m => m.text).join(' ')), true);

// תרגיל משקל גוף בקצה הטווח — לא "להעלות משקל" אלא להקשות
const BWP = { days: [{ name: 'יום A', exercises: [{ name: 'מתח אחיזה רחבה', sets: '3', reps: '8-10' }] }] };
B.load({ logs: [5, 2].map(d => ({ date: ago(d), dayName: 'יום A',
  entries: [{ ex: 'מתח אחיזה רחבה', reps: 10, done: true }] })), program: BWP });
t('משקל גוף — "להקשות" ולא "להעלות משקל"', /זמן להקשות במתח/.test(titles()), true);
t('משקל גוף — בלי המלצת קילוגרמים', /להעלות משקל במתח/.test(titles()), false);
