-- =====================================================================
--  E.B FIT — המתאמן מוסיף לעצמו ארוחות מהספרייה
--  להרצה ב-Supabase: SQL Editor -> New query -> הדבק הכל -> Run
--  ניתן להריץ שוב ושוב בבטחה.
--
--  עמודה נפרדת ולא meals הקיימת: המאמן דוחף את meals בשלמותה בכל
--  סנכרון, ולכן ארוחה שהמתאמן הוסיף לשם הייתה נמחקת בסנכרון הבא.
--  meals_self נכתבת רק מכאן, והמאמן קורא אותה ולעולם לא כותב.
-- =====================================================================

alter table public.trainees
  add column if not exists meals_self jsonb not null default '[]'::jsonb;

-- ---------------------------------------------------------------------
-- 1. שליפה — שתי דרכי הכניסה מחזירות גם את מה שהמתאמן הוסיף
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 2. הוספה — המתאמן מוסיף לעצמו ארוחה מהספרייה
--
--    נשמר רק מזהה הארוחה בספרייה ולא התוכן: הספרייה נשלחת עם
--    האפליקציה, ואם ערך של מרכיב יתוקן בעתיד, ארוחה שנשמרה במלואה
--    הייתה נשארת עם הערך השגוי לנצח.
-- ---------------------------------------------------------------------
...
begin
  ...
  -- כבר קיימת? מחזירים את הרשימה כמות שהיא במקום לשכפל
...
-- 3. הסרה
-- ---------------------------------------------------------------------
create or replace function public.trainee_remove_meal(
  p_token  text,
  p_lib_id text
)
returns jsonb
...
begin
  ...
  update public.trainees
     set meals_self = coalesce((
           select jsonb_agg(e)
             from jsonb_array_elements(coalesce(meals_self,'[]'::jsonb)) e
            where e->>'libId' <> p_lib_id
         ), '[]'::jsonb)
   where id = v_id
   returning meals_self into v_new;
...
revoke all on function public.trainee_remove_meal(text,text)        from public, anon, authenticated;
...
grant execute on function public.trainee_remove_meal(text,text)     to anon, authenticated;
