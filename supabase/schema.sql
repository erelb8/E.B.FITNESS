-- ⚠️ ═══════════════════════════════════════════════════════════════
--  אזהרה: הקובץ הזה אינו בטוח להרצה חוזרת על מסד קיים.
--
--  הוא נכתב לארכיטקטורה שבה המאמן מתחבר דרך Supabase Auth, ולכן
--  הוא מגדיר מדיניות לפי auth.uid() ומעניק הרשאות ל-authenticated.
--  המערכת עברה מאז לזיהוי דרך טוקן מנהל (db-only-admin.sql), שבו
--  התפקיד הוא anon ו-auth.uid() הוא תמיד NULL.
--
--  הרצה של הקובץ הזה על המסד הפעיל מנתקת את המאמן מהנתונים של
--  עצמו: כל כתיבה נדחית ב-403 וכל קריאה מחזירה אפס שורות. זה קרה
--  בפועל ב-14.9.2026 באמצע בניית תוכניות.
--
--  אם הורץ בטעות — supabase/fix-rls-403.sql מחזיר את שתי השכבות,
--  גם המדיניות וגם ההרשאות, ומאמת בסוף מה פעיל.
-- ═══════════════════════════════════════════════════════════════════
